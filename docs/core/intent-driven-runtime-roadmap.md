...
- `SessionStore` 已抽象完整会话 transcript 的 list / read / search / optional write；`JarvisJsonlSessionStore` 定义标准 Jarvis Transcript JSONL v1，文件名带创建时间前缀以支持按时间查找；`GeminiCliSessionStore` 作为旧 Gemini/Jarvis chat 文件兼容 adapter。
