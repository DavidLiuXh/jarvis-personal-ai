    ...
    async execute(context) {
        ...
        return runInDevTraceSpan({
            ...
            try {
                ...
                const isAbortError = executionError instanceof Error &&
                    (executionError.name === 'AbortError' ||
                        executionError.message.includes('Operation cancelled by user'));
                if (signal.aborted || isAbortError) {
                    completedToolCall = await this.createCancelledResult(call, 'User cancelled tool execution.');
                }
                else {
        ...
        if (typeof content === 'string' && toolName === SHELL_TOOL_NAME) {
            ...
            if (typeof firstPart === 'object' && typeof firstPart.text === 'string') {
                ...
                if (threshold > 0 && textContent.length > threshold) {
                    ...
                    const { outputFile: savedPath } = await saveTruncatedToolOutput(textContent, toolName, callId, this.config.storage.getProjectTempDir(), this.config.getSessionId());
                    outputFile = savedPath;
