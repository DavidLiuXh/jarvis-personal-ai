...
`);
    ...
    describe('validateMcpPolicyToolNames', () => {
        ...
        it('should not warn for wildcard MCP rules', () => {
            const warnings = validateMcpPolicyToolNames('my-server', ['tool1', 'tool2'], [{ toolName: 'my-server__*' }]);
            expect(warnings).toHaveLength(0);
        });
        it('should not warn for rules targeting other servers', () => {
            const warnings = validateMcpPolicyToolNames('server-a', ['tool1'], [{ toolName: 'server-b__toolx' }]);
            expect(warnings).toHaveLength(0);
        });
        it('should not warn for tool names far from any discovered tool', () => {
            const warnings = validateMcpPolicyToolNames('my-server', ['tool1', 'tool2'], [{ toolName: 'my-server__completely_different_name' }]);
            expect(warnings).toHaveLength(0);
        });
        ...
        it('should include source in warning when available', () => {
            const warnings = validateMcpPolicyToolNames('my-server', ['tool1'], [{ toolName: 'my-server__tol1', source: 'User: custom.toml' }]);
            expect(warnings).toHaveLength(1);
