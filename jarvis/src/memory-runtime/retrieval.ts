...
export class DefaultMemoryRetriever {
  ...
  async retrieve(contract: MemoryContract): Promise<MemoryRetrievalResult> {
    ...
    const query = contract.query.rewritten || contract.query.raw;
    const [session, facts, entries] = await Promise.all([
      this.retrieveSession(contract, query),
      this.retrieveFacts(contract, query),
      this.retrieveEntries(contract, query),
    ]);
  ...
  private async retrieveSession(contract: MemoryContract, query: string) {
    ...
    const results = await this.options.stores.session.searchSession(query, {
  ...
  private async retrieveEntries(contract: MemoryContract, query: string) {
    ...
    const results = await this.options.stores.entries.searchEntries(query, {
