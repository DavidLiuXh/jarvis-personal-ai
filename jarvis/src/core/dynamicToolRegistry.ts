...
export class DynamicToolRegistry {
      ...
      for (const metaFile of metaFiles) {
        try {
          const content = fs.readFileSync(
            path.join(this.skillsPath, metaFile),
            "utf8",
          );
          ...
          debugLogger.error(
            `[DynamicToolRegistry] Failed to load skill meta: ${metaFile}`,
            e,
          );
        }
      }
  ...
  public async runSkill(fullName: string, args: any): Promise<string> {
    ...
    ) as EvolvedSkillMetadata;
    const scriptPath = path.join(this.skillsPath, meta.executable);

    if (!fs.existsSync(scriptPath)) {
      throw new Error(
        `Executable for ${skillName} missing: ${meta.executable}`,
      );
    }

    debugLogger.debug(
      `[DynamicToolRegistry] Executing ${skillName} with args:`,
      args,
    );
    ...
    try {
      ...
      debugLogger.error(
        `[DynamicToolRegistry] Skill execution failed: ${errorMsg}`,
      );
