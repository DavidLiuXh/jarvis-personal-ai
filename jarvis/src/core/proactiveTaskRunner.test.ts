...
function makeAgent(responseText: string, shouldError = false) {
  ...
  agent.processMessage = vi.fn().mockImplementation(async () => {
    if (shouldError) {
      ...
      agent.emit(JarvisEventType.CONTENT, {
...
function makeRegistry(pushSafeResult = true) {
  return {
    ...
    pushSafe: vi.fn().mockResolvedValue(pushSafeResult),
  } as any;
}
...
describe("ProactiveTaskRunner", () => {
  it("runs a task and pushes result to the channel via pushSafe", async () => {
    ...
    expect(registry.pushSafe).toHaveBeenCalledWith(
      "feishu",
      "oc_1",
  ...
  it("passes proactive execution context and avoids schedule wording in delivery prompt", async () => {
    const agent = makeAgent("Report");
    const getAgent = vi.fn().mockResolvedValue(agent);
    const registry = makeRegistry();
    registry.isRegistered = vi.fn().mockReturnValue(true);

    const runner = new ProactiveTaskRunner(getAgent);
    const task = {
      id: "daily-ai-report",
      prompt: "Generate the daily AI report",
      channel: "wechat",
      cron: "",
      enabled: true,
    };
    ...
    const prompt = agent.processMessage.mock.calls[0]?.[0] as string;
    expect(prompt).toContain("[PROACTIVE TASK");
    expect(prompt).toContain("delivery channel: wechat");
    expect(prompt).not.toContain("SCHEDULED TASK");
    expect(agent.processMessage).toHaveBeenCalledWith(prompt, undefined, {
      executionContext: "proactive_task",
    });
  });

  it("falls back to websocket when configured task channel is not registered", async () => {
    const agent = makeAgent("Fallback result");
    const getAgent = vi.fn().mockResolvedValue(agent);
    const registry = makeRegistry();
    registry.isRegistered = vi.fn((channel: string) => channel === "websocket");
