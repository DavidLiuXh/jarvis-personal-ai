...
import { TaskScheduler, type TaskConfig, type TasksConfig } from './taskScheduler.js';

function writeTasks(dir: string, config: TasksConfig) {
  fs.writeFileSync(path.join(dir, 'tasks.json'), JSON.stringify(config, null, 2));
}
...
describe('TaskScheduler', () => {
  ...
  it('returns empty task list when tasks.json does not exist', () => {
    const scheduler = new TaskScheduler(tmpDir);
  ...
  it('calls onTrigger callback when a task fires', async () => {
    ...
    (cron.default.schedule as any).mockImplementation((_expr: string, cb: () => void) => {
      capturedCallback = cb;
      return { stop: vi.fn() };
    });
  ...
  it('uses task-level chatId over default when specified', async () => {
    ...
    writeTasks(tmpDir, {
      ...
      tasks: [
        { id: 'task1', cron: '0 8 * * *', prompt: 'Brief', enabled: true, chatId: 'oc_custom' },
      ],
    });
