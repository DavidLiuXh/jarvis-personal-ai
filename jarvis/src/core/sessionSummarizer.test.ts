...
describe("buildIncrementalSummary", () => {
  it("generates a new summary when no prior summary exists", async () => {
    const generateText = vi
      .fn()
      .mockResolvedValue("User likes cycling and is named David.");
    const result = await buildIncrementalSummary(
      fakeMessages,
      null,
      generateText,
    );
  ...
  it("merges existing summary with new messages", async () => {
    const generateText = vi
      .fn()
      .mockResolvedValue("Updated summary with new info.");
  ...
  it("merge prompt instructs to overwrite old facts when new info conflicts", async () => {
    ...
    expect(prompt.toLowerCase()).toMatch(
      /contradict|conflict|overwrite|supersede|newer/,
    );
  });
...
describe("buildHistoryWithSummary", () => {
  it("prepends summary as a model turn before recent messages", () => {
    ...
    expect((history[0].parts[0] as any).text).toContain(
  ...
  it("returns only recent turns when summary is empty", () => {
    ...
    // No summary prefix
    expect(history[0].role).toBe("user");
    expect((history[0].parts[0] as any).text).not.toContain(
...
describe("buildChunkedRollingSummary", () => {
  it("re-compresses rolling summary between chunks when it exceeds max length", async () => {
    const messages: SessionMessage[] = [
      { type: "user", content: "message 1" },
      { type: "gemini", content: "reply 1" },
      { type: "user", content: "message 2" },
      { type: "gemini", content: "reply 2" },
    ];
    const longSummary = "x".repeat(120);
    const generateText = vi
      .fn()
      .mockResolvedValueOnce(longSummary)
      .mockResolvedValueOnce("short summary")
      .mockResolvedValueOnce("final summary");

    const result = await buildChunkedRollingSummary(
      messages,
      null,
      generateText,
      {
    ...
    expect(generateText.mock.calls[1][0]).toContain(
      "Compress the following session summary",
    );
    expect(generateText.mock.calls[2][0]).toContain("short summary");
    expect(generateText.mock.calls[2][0]).not.toContain(longSummary);
  });
});
      ...
      entities: [
        {
          type: "person",
          name: "David",
          attrs: { profession: "software engineer" },
        },
      ],
...
describe("mergeStructuredContext", () => {
  it("merges new entities into existing context without duplicates", () => {
    const existing: StructuredContext = {
      entities: [
        { type: "person", name: "David", attrs: { profession: "engineer" } },
      ],
      behaviors: [{ content: "runs 3 times a week", confidence: "high" }],
      decisions: [],
      preferences: [],
    ...
    const incoming: StructuredContext = {
      entities: [
        { type: "person", name: "David", attrs: { hobby: "cycling" } },
      ],
    ...
    // New behavior added
    expect(
      merged.behaviors.some((b) => b.content.includes("reads books")),
    ).toBe(true);
    // Decision added
      ...
      projects: [
        {
          name: "Jarvis",
          status: "active",
          key_rules: ["do not modify gemini-cli source"],
        },
      ],
    };
